 ...
 *   NOTE: referenced wiki.osdev.org/RTC#Interrupts_and_Registers_C
 */
void rtchandler()
{
  uint32_t i;
  ...
  for (i=0; i<MAX_PROCESS; i++) {
    if (++rtc_proc_ctr[i] >= rtc_proc_ctr_max[i]) {
      rtc_proc_ctr[i] = 0;    // reset counter
      rtc_proc_intr[i] = 1;   // generate interrupt
    }
  }
